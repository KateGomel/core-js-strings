...
function removeFirstOccurrences(str, value) {
  ...
  const char = str.indexOf(value);
  return str.slice(0, char) + str.slice(char + value.length, str.length);
}
...
function removeLastOccurrences(str, value) {
  ...
  const char = str.lastIndexOf(value);
  return str.slice(0, char) + str.slice(char + value.length, str.length);
}
...
function formatTime(minutes, seconds) {
  const ZeroMinutes = String(minutes).padStart(2, '0');
  const ZeroSeconds = String(seconds).padStart(2, '0');
  return `${ZeroMinutes}:${ZeroSeconds}`;
}
